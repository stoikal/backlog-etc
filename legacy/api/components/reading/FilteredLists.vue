...
const searchKeyApplied = ref('')

const debouncedChangeSearchKey = useDebounceFn((value) => {
  searchKeyApplied.value = value
}, 600)

watch(searchKeyTemp, (value) => {
  debouncedChangeSearchKey(value)
})

watch(() => props.lists, (value) => {
  filteredLists.value = value
})

const filteredLists = computed(() => {
  const isFilterEmpty = searchKeyApplied.value.length === 0

  if (isFilterEmpty) return props.lists
  ...
  return props.lists
    .map((list) => ({
      ...
      items: list.items
        .filter((item) => {
          const itemName = item.workTitle.toLowerCase()
          
          return itemName.includes(searchKeyApplied.value.trim().toLowerCase())
        })
    }))
